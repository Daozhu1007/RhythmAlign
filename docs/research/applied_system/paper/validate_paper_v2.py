"""Read-only paper/evidence checks for PAPER-DRAFT-2; --write-v2 writes only the
three generated V2 paper files (PAPER_DRAFT_V2.md, PAPER_TABLES_V2.md,
CLAIM_LEDGER_V2.md).

No comparator imports, audio access, aggregation reruns, or frozen-file writes.
Every validate_paper_v1.py check is preserved (same sealed projections of
reporting v2, the immutable final-results artifact, and the corrected
gcc_phat_v2_lagfix results; all SHA-256/raw-record hashes; the revision-1
wording gate). On top sit the second-review gates:

- Issue A (GCC wording): required support-geometry clarification in §4.5; no
  statement that the PHAT-weighted curve vanishes in the physical zero-overlap
  index region or that out-of-support PHAT values are numerical roundoff; a
  required clarifying addendum in the preserved GCC lag audit, asserted purely
  additive against the pre-revision HEAD.
- Issue B (NCC counterfactual): the overbroad "guard alone accounts for the
  NCC contrast" statement must be gone; the required counterfactual scoping
  (a 30 s-constrained NCC was not evaluated) must appear in §6.4, §7, and the
  Table 7 NCC overlap contract panel.
- Change containment: PAPER_DRAFT_V2.md, PAPER_TABLES_V2.md, and
  CLAIM_LEDGER_V2.md must equal their V1 counterparts plus exactly the
  enumerated wording edits below (all decimal-bearing tokens identical to V1);
  every V1 paper artifact is untouched; frozen evidence is untouched; and the
  diff versus the pre-revision HEAD is documentation-only.

SECOND_REVIEW_RESPONSE_V2.md and HOSTILE_REVIEW_RESPONSE_V1_1.md are response
memos: they quote removed wording in order to document it, so forbidden-wording
scans cover the manuscript set (draft, tables, claim ledger, citation ledger,
citation gaps, first-review response memo), exactly as in revision 1; the
memos instead get required-content gates.
"""
...
BASE = "ed337e9"  # pre-V2 HEAD (PAPER-DRAFT-1 + corrected review-response mapping)
AUDIT_REL = "docs/research/applied_system/GCC_PHAT_LAG_AUDIT.md"
SR = 48000
SYSTEMS = {
...
NCC_COUNTERFACTUAL_SENTENCE = (
    "Because a 30 s-constrained NCC was not evaluated, this observation does "
    "not determine the counterfactual performance of a matched NCC system."
)
...
# ---------------------------------------------------------------------------
# Enumerated V1 -> V2 wording edits (the single source of truth; --write-v2
# applies them, validate() proves the V2 files contain nothing else).
# ---------------------------------------------------------------------------

DRAFT_EDITS = [
    # Status line (task §6).
    ("STATUS: TECHNICAL-CORE DRAFT V1 — REVISED AFTER INDEPENDENT HOSTILE REVIEW AND "
     "COMPARATOR-INTEGRITY AUDITS; SECOND REVIEW PENDING; CREATOR FEASIBILITY PILOT NOT CONDUCTED.",
     "STATUS: SCIENTIFIC-CONTENT FINAL CANDIDATE — SECOND REVIEW MAJOR ISSUES RESOLVED; "
     "NO FURTHER EXPERIMENTS REQUIRED FOR CURRENT SCOPE; VENUE FORMATTING PENDING."),
    # Header: versioning and readiness sentence.
    ("Revised manuscript, PAPER-DRAFT-1, supersedes PAPER-DRAFT-0 (kept as revision "
     "history). Not submission-ready.",
     "Second-review revised manuscript, PAPER-DRAFT-2, supersedes PAPER-DRAFT-1 (kept as "
     "revision history). Scientific content is final for the current scope; venue "
     "formatting remains pending and submission readiness is not claimed."),
    # Header: companion links move to V2 + both response memos.
    ("Companion [tables](PAPER_TABLES_V1.md), [claim ledger](CLAIM_LEDGER_V1.md), "
     "[citation ledger](CITATION_LEDGER_V1.md), [citation-gap status](CITATION_GAPS_V1.md), "
     "and the [review response memo](HOSTILE_REVIEW_RESPONSE_V1.md) preserve the evidence "
     "and revision boundaries.",
     "Companion [tables](PAPER_TABLES_V2.md), [claim ledger](CLAIM_LEDGER_V2.md), "
     "[citation ledger](CITATION_LEDGER_V1.md), [citation-gap status](CITATION_GAPS_V1.md), "
     "the [first-review response memo](HOSTILE_REVIEW_RESPONSE_V1_1.md), and the "
     "[second-review response memo](SECOND_REVIEW_RESPONSE_V2.md) preserve the evidence "
     "and revision boundaries."),
    # Issue A: support-geometry clarification in §4.5 (no PHAT zero-overlap claim).
    ("changes only the lag mapping. A uniform rerun",
     "changes only the lag mapping. The corrected unwrap follows the physically "
     "meaningful linear-lag support determined by the two signal lengths; the original "
     "midpoint unwrap was invalid for unequal lengths. One boundary requires care: the "
     "ordinary linear cross-correlation is algebraically zero in the index region where "
     "the zero-padded signals do not overlap, but PHAT weighting does not preserve those "
     "zeros exactly, and this paper makes no claim about PHAT-weighted values outside "
     "the physical overlap support. None of the 50 stored final GCC argmax indices fell "
     "in that physical zero-overlap index region, so this subtlety does not alter any "
     "measured v2 final outcome. A uniform rerun"),
    # Issue B: §6.4 exclusion statement + counterfactual scoping.
    ("RhythmAlign's edge-lag guard alone excludes all five placements, independent of "
     "its other gates.",
     "RhythmAlign's 30 s candidate-overlap requirement excludes all five observed "
     "erroneous NCC placements, independent of its other gates. " + NCC_COUNTERFACTUAL_SENTENCE),
    # Issue B: Discussion overbroad sentence removed and replaced (task §3).
    ("The result is consistent with selective handling of difficult evidence, but this "
     "system-level benchmark cannot assign causality to any individual gate, and the "
     "30 s edge-lag guard alone accounts for the NCC contrast of Section 6.4.",
     "The result is consistent with selective handling of difficult evidence, but this "
     "system-level benchmark cannot assign causality to any individual gate. "
     "RhythmAlign's 30 s candidate-overlap requirement excludes all five observed "
     "erroneous NCC placements of Section 6.4. " + NCC_COUNTERFACTUAL_SENTENCE),
    # §9 points at the current checker.
    ("The [validation script](validate_paper_v1.py) performs such recomputation",
     "The [validation script](validate_paper_v2.py) performs such recomputation"),
    # §10: the second review has now happened (no further experiments for current scope).
    ("unmeasured creator recovery cost; a second-round review precedes any decision "
     "about human data.",
     "unmeasured creator recovery cost. The completed second review required no further "
     "experiments for the current scope; any human-data extension remains future work "
     "requiring its own protocol and review."),
]

TABLE_EDITS = [
    ("# Paper Tables V1", "# Paper Tables V2"),
    ("STATUS: TECHNICAL-CORE TABLES V1 — revised after comparator-integrity review; "
     "sealed benchmark evidence; creator pilot NOT_CONDUCTED.",
     "STATUS: SCIENTIFIC-CONTENT FINAL CANDIDATE TABLES V2 — second-review wording "
     "revision only; every table identical to PAPER_TABLES_V1.md (kept as revision "
     "history); sealed benchmark evidence; creator pilot NOT_CONDUCTED."),
    ("All tables are projections of stored evidence, not new performance analyses. "
     "Fractions retain exact denominators; displayed decimals are rounded.",
     "All tables are projections of stored evidence, not new performance analyses. "
     "Fractions retain exact denominators; displayed decimals are rounded. V2 is a "
     "wording-only revision: every table and every number below is identical to "
     "PAPER_TABLES_V1.md; the only textual change is the added counterfactual-scoping "
     "sentence in the NCC overlap contract panel."),
    ("RhythmAlign's edge-lag guard alone excludes all five placements. Source: "
     "NCC_OVERLAP_CONTRACT_AUDIT.md;",
     "RhythmAlign's edge-lag guard alone excludes all five placements. "
     + NCC_COUNTERFACTUAL_SENTENCE + " Source: NCC_OVERLAP_CONTRACT_AUDIT.md;"),
]

LEDGER_EDITS = [
    ("STATUS: BENCHMARK-ONLY CLAIM PACKAGE, REVISED AFTER COMPARATOR-INTEGRITY REVIEW. "
     "Creator feasibility pilot: NOT_CONDUCTED.",
     "STATUS: BENCHMARK-ONLY CLAIM PACKAGE, REVISED AFTER THE SECOND INDEPENDENT REVIEW "
     "(WORDING-ONLY REVISION). Creator feasibility pilot: NOT_CONDUCTED. No result, "
     "threshold, frozen artifact, or experiment changed."),
    ('This ledger supersedes CLAIM_LEDGER_V0 (kept as revision history) and constrains '
     'subsequent manuscript revisions. "Allowed" refers to this collected benchmark and '
     'the frozen implementations. It does not authorize a broader empirical or '
     'algorithmic claim. The manuscript is a technical-core draft pending a second '
     'independent review, not submission-ready.',
     'This ledger supersedes CLAIM_LEDGER_V1 (kept as revision history), which '
     'superseded CLAIM_LEDGER_V0, and constrains subsequent manuscript revisions. '
     '"Allowed" refers to this collected benchmark and the frozen implementations. It '
     'does not authorize a broader empirical or algorithmic claim. The manuscript is a '
     'scientific-content-final candidate for the current scope: the second independent '
     'review returned TECHNICAL_CORE_DEFENSIBLE_WITH_MINOR_REVISION with no further '
     'experiments required, and only the two wording issues below were corrected. Venue '
     'formatting remains pending.'),
    # L30 evidence cell: the operative zero-overlap fact.
    ("zero scored outcome changes at 0.05/0.1/0.15 s; v1 runner/records preserved verbatim",
     "zero scored outcome changes at 0.05/0.1/0.15 s; v1 runner/records preserved "
     "verbatim; none of the 50 stored argmax indices fell in the physical zero-overlap "
     "index region (GA §6 records 0 there)"),
    # L30 forbidden-strengthening cell: Issue A boundary.
    ('"Bug confirmed, corrected synthetic-first, zero scored outcome changes"; never '
     'present the correction as misconduct evidence or silently replace v1',
     '"Bug confirmed, corrected synthetic-first, zero scored outcome changes"; never '
     'present the correction as misconduct evidence or silently replace v1; never '
     'transfer the ordinary correlation\'s zero-overlap identity to the PHAT-weighted '
     'curve, never characterize out-of-support PHAT values as mere implementation dust, '
     'and never claim the final benchmark validates a general out-of-support theorem '
     '(second-review Issue A)'),
    # L31 evidence cell: point at the current checker.
    ("recomputed by validate_paper_v1.py; pinned by tests/test_comparator_integrity_audit.py",
     "recomputed by validate_paper_v2.py; pinned by tests/test_comparator_integrity_audit.py"),
    # L31 forbidden-strengthening cell: Issue B boundary.
    ("never conclude NCC inherently unreliable, correlation cannot solve the task, or "
     "that a 30 s-filtered NCC would succeed (untested system)",
     "never conclude NCC inherently unreliable, correlation cannot solve the task, or "
     "that a 30 s-filtered NCC would succeed (untested system); never attribute the NCC "
     "contrast to the guard by itself and never assert a counterfactual about a 30 "
     "s-constrained NCC — established facts: all five OBSERVED wrong positives sit "
     "below 30 s of usable overlap and the frozen policy excludes those five observed "
     "placements, while the counterfactual performance of a matched 30 s-constrained "
     "NCC was not evaluated and is not determined (second-review Issue B)"),
    ("The abstract contains ten sentences. `validate_paper_v1.py` compares the entire abstract",
     "The abstract contains ten sentences. `validate_paper_v2.py` compares the entire abstract"),
    ("`validate_paper_v1.py` projects all seven publication tables from sealed records "
     "(including the GCC v2 distributions from G2 and the paired counts from F), "
     "verifies the main table duplicated in the manuscript, verifies rounded result "
     "prose, recomputes the five NCC wrong-positive overlaps from frozen lag samples "
     "and manifest lengths, validates citation IDs R1–R14 and local links, checks every "
     "SHA-256 sidecar and all 200 frozen per-record hashes, enforces the revision-1 "
     "wording gate (title, comparator labeling, superseded maximum, NCC disclosure, "
     "30 s rule, no causal-abstention claim, no consequence-class wording, no "
     "workflow-benefit wording, resolved TODOs, prior art present, scoped "
     "reproducibility, no invented pilot), and rejects tracked edits outside this paper "
     "directory relative to `8940cc4`. It never calls a comparator or reads audio.",
     "`validate_paper_v2.py` preserves every `validate_paper_v1.py` numeric, hash, and "
     "citation check and adds the second-review gates: Issue A (required "
     "support-geometry clarification; no statement that the PHAT-weighted curve "
     "vanishes in the physical zero-overlap index region or that out-of-support PHAT "
     "values are mere implementation dust; a required clarifying addendum in the GCC lag "
     "audit, asserted purely additive), Issue B (no statement that the 30 s guard by "
     "itself accounts for or fully explains the NCC contrast; required counterfactual "
     "scoping that a 30 s-constrained NCC was not evaluated), proof that the V1→V2 "
     "manuscript/table/ledger diffs are exactly the enumerated wording edits with "
     "numerical identity to V1, V1 artifact preservation, and a documentation-only diff "
     "relative to `ed337e9` (paper directory plus the GCC audit's clarifying addendum). "
     "It never calls a comparator or reads audio."),
    ("Run with `python docs/research/applied_system/paper/validate_paper_v1.py`.",
     "Run with `python docs/research/applied_system/paper/validate_paper_v2.py`."),
    # Append the second-review receipt after the V1 receipt (kept as history).
    ("- Verdict: **TECHNICAL_CORE_V1_READY_FOR_SECOND_REVIEW**. Submission readiness is "
     "not claimed. Creator feasibility pilot: **NOT_CONDUCTED**; recovery cost remains "
     "unmeasured.",
     "- Verdict: **TECHNICAL_CORE_V1_READY_FOR_SECOND_REVIEW**. Submission readiness is "
     "not claimed. Creator feasibility pilot: **NOT_CONDUCTED**; recovery cost remains "
     "unmeasured.\n\n"
     "## Second-review response and PAPER-DRAFT-2 receipt — 2026-09-20\n\n"
     "- Second independent review verdict: **TECHNICAL_CORE_DEFENSIBLE_WITH_MINOR_"
     "REVISION**; MORE_EXPERIMENTS_REQUIRED: **NO**; pilot: **PILOT_NOT_NEEDED_FOR_"
     "CURRENT_SCOPE**. All first-review issues M1–M4 and m1–m3 remain resolved "
     "([HOSTILE_REVIEW_RESPONSE_V1_1.md](HOSTILE_REVIEW_RESPONSE_V1_1.md)).\n"
     "- **Issue A (GCC wording).** §4.5 now states that the corrected v2 unwrap follows "
     "the physically meaningful linear-lag support determined by the two signal "
     "lengths, that the original midpoint unwrap was invalid for unequal lengths, that "
     "PHAT weighting does not preserve the ordinary zero-overlap zeros, that no claim "
     "is made about PHAT-weighted values outside the physical overlap support, and that "
     "none of the 50 stored final GCC argmax indices fell in the physical zero-overlap "
     "index region — so the subtlety does not alter any measured v2 final outcome. L30 "
     "forbids transferring the ordinary zero-overlap identity to the PHAT-weighted "
     "curve. The preserved GCC audit received a purely additive clarifying addendum "
     "(§12); no recorded value, hash, prediction, or outcome changed.\n"
     "- **Issue B (NCC counterfactual).** The Discussion sentence asserting that the "
     "30 s edge-lag guard by itself accounts for the NCC contrast is removed. §6.4, "
     "§7, and the Table 7 NCC overlap contract panel now state the established facts — "
     "RhythmAlign's 30 s candidate-overlap requirement excludes all five observed "
     "erroneous NCC placements, and because a 30 s-constrained NCC was not evaluated, "
     "the observation does not determine the counterfactual performance of a matched "
     "NCC system. L31 records the prohibition. The retained statement that the "
     "comparison reflects estimator and candidate-policy differences and does not "
     "isolate the causal effect of abstention is unchanged.\n"
     "- No new data; no result, threshold, frozen artifact, protocol, source set, or "
     "production-code change; no comparator executed. The V1→V2 manuscript/table/ledger "
     "diffs are the enumerated wording edits proven by `validate_paper_v2.py`; all "
     "numbers are identical to V1. Citation TODOs remain 0 (CITATION_LEDGER_V1 "
     "unchanged; no literature expansion).\n"
     "- Paper checker: PASS — all V1 checks preserved plus the second-review gates "
     "(see the script output for this revision's counts).\n"
     "- Verdict: **SCIENTIFIC_CONTENT_FINAL_CANDIDATE**. Venue formatting pending; "
     "submission readiness not claimed. Creator feasibility pilot: **NOT_CONDUCTED**."),
]

GENERATED = [("PAPER_DRAFT_V2.md", "PAPER_DRAFT_V1.md", DRAFT_EDITS),
             ("PAPER_TABLES_V2.md", "PAPER_TABLES_V1.md", TABLE_EDITS),
             ("CLAIM_LEDGER_V2.md", "CLAIM_LEDGER_V1.md", LEDGER_EDITS)]

# --- Second-review gate constants -------------------------------------------

ISSUE_A_FORBIDDEN = [
    r"must be zero", r"exactly zero", r"guaranteed to be zero",
    r"numerical roundoff", r"curve is zero",
    r"preserves ordinary zero-overlap zeros",
    r"empirically validates a general theorem",
]
ISSUE_B_FORBIDDEN = [
    r"guard alone accounts", r"alone accounts for", r"alone explains",
    r"guard alone explains", r"guard would make", r"would make NCC",
]
ISSUE_A_REQUIRED_DRAFT = [
    "STATUS: SCIENTIFIC-CONTENT FINAL CANDIDATE — SECOND REVIEW MAJOR ISSUES RESOLVED; "
    "NO FURTHER EXPERIMENTS REQUIRED FOR CURRENT SCOPE; VENUE FORMATTING PENDING.",
    "physically meaningful linear-lag support determined by the two signal lengths",
    "the original midpoint unwrap was invalid for unequal lengths",
    "PHAT weighting does not preserve those zeros exactly",
    "this paper makes no claim about PHAT-weighted values outside the physical overlap support",
    "None of the 50 stored final GCC argmax indices fell in that physical zero-overlap index region",
    "does not alter any measured v2 final outcome",
    "submission readiness is not claimed",
]
ISSUE_A_REQUIRED_AUDIT = [
    "Clarifying addendum (second review",
    "sections 1–11 above are preserved verbatim",
    "PHAT weighting does not preserve those zeros exactly",
    "not mathematically guaranteed to be zero",
    "no claim — general or empirical — about PHAT-weighted values outside the physical overlap support",
    "immaterial to every recorded and scored v2 outcome",
]
AUDIT_FORBIDDEN = [r"must be zero", r"must be numerical roundoff", r"values must be"]
ISSUE_A_REQUIRED_LEDGER = [
    "never transfer the ordinary correlation's zero-overlap identity to the PHAT-weighted curve",
    "never characterize out-of-support PHAT values as mere implementation dust",
    "never claim the final benchmark validates a general out-of-support theorem",
    "Issue A (GCC wording)",
    "purely additive clarifying addendum",
]
ISSUE_B_REQUIRED_LEDGER = [
    "never attribute the NCC contrast to the guard by itself",
    "the counterfactual performance of a matched 30 s-constrained NCC was not evaluated "
    "and is not determined",
    "Issue B (NCC counterfactual)",
]
RESPONSE_FILE = "SECOND_REVIEW_RESPONSE_V2.md"
RESPONSE_REQUIRED = [
    "TECHNICAL_CORE_DEFENSIBLE_WITH_MINOR_REVISION",
    "MORE_EXPERIMENTS_REQUIRED: **NO**",
    "PILOT_NOT_NEEDED_FOR_CURRENT_SCOPE",
    "M1–M4", "m1–m3", "Issue A", "Issue B",
    "No new data", "no result changed", "no experiment",
    "no literature expansion", "CITATION_LEDGER_V1",
]
...
def paper_texts():
    return {name: (HERE / name).read_text(encoding="utf-8") for name in [
        ...
        "PAPER_DRAFT_V2.md", "PAPER_TABLES_V2.md", "CLAIM_LEDGER_V2.md",
        RESPONSE_FILE,
    ]}
...
def apply_edits(text, edits, label):
    for index, (old, new) in enumerate(edits):
        ...
        assert count == 1, (
            f"{label} edit #{index}: expected exactly one occurrence, found {count}: {old[:90]!r}")
        text = text.replace(old, new)
    return text


def result_decimals(text):
    # Decimal-bearing result tokens; structural section references (§4.5,
    # Section 6.4) are not results and are removed before comparison.
    text = re.sub(r"§+ ?\d+\.\d+", "", text)
    text = re.sub(r"[Ss]ections? \d+\.\d+", "", text)
    return re.findall(r"\d+\.\d+", text)


def validate():
    files = paper_texts()
    draft = files["PAPER_DRAFT_V2.md"]
    tables = files["PAPER_TABLES_V2.md"]
    ledger = files["CLAIM_LEDGER_V2.md"]
    response = files[RESPONSE_FILE]
    v1_draft = files["PAPER_DRAFT_V1.md"]

    # --- V1 artifacts preserved verbatim; V1 table projection still exact ---
    assert tables is not None and files["PAPER_TABLES_V1.md"] == render_tables(), \
        "PAPER_TABLES_V1.md drift from sealed summaries"
    for name in ["PAPER_DRAFT_V1.md", "PAPER_TABLES_V1.md", "CLAIM_LEDGER_V1.md",
                 "CITATION_LEDGER_V1.md", "CITATION_GAPS_V1.md", "HOSTILE_REVIEW_RESPONSE_V1.md"]:
        diff = subprocess.check_output(
            ["git", "diff", BASE, "--", f"docs/research/applied_system/paper/{name}"],
            cwd=ROOT, text=True)
        assert diff == "", f"V1 artifact modified relative to {BASE}: {name}"

    # --- Enumerated-edit proof: V2 == V1 + listed wording edits, nothing else ---
    for v2_name, v1_name, edits in GENERATED:
        base_text = files[v1_name]
        expected = apply_edits(base_text, edits, v2_name)
        assert files[v2_name] == expected, \
            f"{v2_name} differs from {v1_name} beyond the enumerated wording edits"
        assert result_decimals(files[v2_name]) == result_decimals(base_text), \
            f"{v2_name}: decimal-bearing result tokens changed"

    # --- Main table duplicated in the V2 manuscript ---
    ...
    # --- GCC v2 comparator integrity (revision-1 gate, unchanged) ---
    cases = gcc_v2_positive_cases()
    produced, correct, wrong_refs = gcc_v2_distributions()
    assert len(cases) == 24
    ...
    # --- Abstract audit: identical to V1, still evidence-parameterized ---
    abstract = draft.split("## Abstract\n", 1)[1].split("\n## 1.", 1)[0].strip()
    assert abstract == " ".join(abstract_expected()), "Abstract needs a fresh complete claim audit"
    v1_abstract = v1_draft.split("## Abstract\n", 1)[1].split("\n## 1.", 1)[0].strip()
    assert abstract == v1_abstract, "Abstract changed between V1 and V2"
    ...
    # --- NCC overlap-contract recomputation and disclosure (revision-1 gate) ---
    ...
    # --- Second-review Issue A gates (GCC zero-overlap wording) ---
    for phrase in ISSUE_A_REQUIRED_DRAFT:
        assert phrase in draft, f"Issue A required in draft: {phrase}"
    audit = (ROOT / AUDIT_REL).read_text(encoding="utf-8")
    audit_flat = " ".join(audit.split())  # the audit hard-wraps lines
    for phrase in ISSUE_A_REQUIRED_AUDIT:
        assert phrase in audit_flat, f"Issue A required in GCC audit addendum: {phrase}"
    for phrase in ISSUE_A_REQUIRED_LEDGER:
        assert phrase in ledger, f"Issue A required in claim ledger: {phrase}"
    audit_diff = subprocess.check_output(["git", "diff", BASE, "--", AUDIT_REL],
                                         cwd=ROOT, text=True)
    removed = [ln for ln in audit_diff.splitlines()
               if ln.startswith("-") and not ln.startswith("---")]
    assert not removed, f"GCC audit edit is not purely additive: {removed[:3]}"

    # --- Second-review Issue B gates (NCC counterfactual wording) ---
    assert draft.count(NCC_COUNTERFACTUAL_SENTENCE) == 2, \
        "Counterfactual scoping must appear exactly in §6.4 and §7"
    assert draft.count("excludes all five observed erroneous NCC placements") == 2
    assert NCC_COUNTERFACTUAL_SENTENCE in tables, "Table 7 panel counterfactual scoping missing"
    for phrase in ISSUE_B_REQUIRED_LEDGER:
        assert phrase in ledger, f"Issue B required in claim ledger: {phrase}"

    # --- Numeric prose guards (revision-1 set, on the V2 manuscript) ---
    ...
    assert "136.9–139.3 s" in draft and "62.9–68.7 s" in draft
    assert "final21–final22 to the primary device D1" in draft
    ...
    # --- Second-review response memo required content ---
    for phrase in RESPONSE_REQUIRED:
        assert phrase in response, f"Second-review response memo missing: {phrase}"

    # --- Revision-1 forbidden/required wording gate (V2 manuscript set) ---
    combined = "\n".join([draft, tables, ledger, files["CITATION_LEDGER_V1.md"],
                          files["CITATION_GAPS_V1.md"], files["HOSTILE_REVIEW_RESPONSE_V1.md"]])
    assert f"# {TITLE}" in draft
    assert "SECOND REVIEW PENDING" not in draft
    ...
    assert "Public Audio Identification Evaluation Framework" in draft
    ...
    assert "pilot exists but was not conducted" in draft and "NOT_CONDUCTED" in ledger
    assert "Positive-case accepted error rate" in draft and "Accepted risk" not in draft.split("## References")[0].split("## Abstract")[1]
    for pattern in ISSUE_A_FORBIDDEN + ISSUE_B_FORBIDDEN:
        assert not re.search(pattern, draft, flags=re.I), f"Issue gate tripped in draft: {pattern}"
        assert not re.search(pattern, tables, flags=re.I), f"Issue gate tripped in tables: {pattern}"
        assert not re.search(pattern, ledger, flags=re.I), f"Issue gate tripped in ledger: {pattern}"
    for pattern in AUDIT_FORBIDDEN:
        assert not re.search(pattern, audit_flat, flags=re.I), f"Audit gate tripped: {pattern}"
    ...
    # --- Documentation-only diff vs BASE; GCC audit additive-only (asserted above) ---
    ...
    outside = {p for p in changed
               if not p.startswith("docs/research/applied_system/paper/")
               and p != ".zcodeignore" and p != AUDIT_REL}
    assert not outside, outside
    body = draft.split("## Abstract", 1)[1].split("## References", 1)[0]
    print(f"PASS: V2 == V1 + {len(DRAFT_EDITS)}/{len(TABLE_EDITS)}/{len(LEDGER_EDITS)} enumerated "
          f"wording edits (draft/tables/ledger) with identical decimal tokens; V1 artifacts and frozen "
          f"evidence unchanged; seven table projections; abstract identical to V1 ({abstract_words} words); "
          f"GCC v2 integrity; NCC overlap recomputation; Issue A gates (support-geometry clarification, "
          f"no PHAT zero-overlap claim, audit addendum purely additive); Issue B gates (overbroad sentence "
          f"removed, counterfactual scoping in §6.4/§7/Table 7 panel); response-memo content; revision-1 "
          f"wording gate; citation IDs R1–R14; links; {len(sidecars)} SHA-256 sidecars; "
          f"{len(raw['records'])} raw record hashes; documentation-only diff vs {BASE}.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--write-v2", action="store_true",
                        help="write the three generated V2 files from V1 plus the enumerated edits")
    args = parser.parse_args()
    if args.write_v2:
        for v2_name, v1_name, edits in GENERATED:
            base_text = (HERE / v1_name).read_text(encoding="utf-8")
            (HERE / v2_name).write_text(apply_edits(base_text, edits, v2_name),
                                        encoding="utf-8", newline="\n")
        print("Wrote PAPER_DRAFT_V2.md, PAPER_TABLES_V2.md, CLAIM_LEDGER_V2.md "
              "from V1 plus the enumerated wording edits.")
    else:
        validate()
